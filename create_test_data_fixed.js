...
async function createTestDataFixed() {
  console.log('🔧 Creating Test Data (Fixed Version)...\n');
  ...
  try {
    ...
    // Step 2: Create test users/profiles with proper UUID
    ...
    // Step 4: Create test schedules with proper time format
    ...
    // Step 5: Create test time entries (checking schema first)
    console.log('\n5️⃣ Creating test time entries...');
    const today = new Date();
    today.setHours(8, 0, 0, 0);

    if (driverProfile) {
      // First, let's check what columns exist in time_entries
      const { data: timeEntryTest, error: timeEntryTestError } = await supabase
        .from('time_entries')
        .select('*')
        .limit(1);

      if (timeEntryTestError) {
        console.log('ℹ️  Time entries table schema issue:', timeEntryTestError.message);
      } else {
        console.log('✅ Time entries table accessible');
        
        // Try to create a time entry with basic fields
    ...
    console.log('\n💡 Note: You may need to create auth users separately');
    console.log('   The profiles are created but auth users need to be added');
