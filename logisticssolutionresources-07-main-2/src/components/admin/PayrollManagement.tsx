...
const PayrollManagement = () => {
  ...
  // Fetch real payroll data from time entries
  const { data: payrollData = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch actual time entries for the period
      const { data: timeEntries, error } = await supabase
        .from('time_entries')
        .select('*, driver_id, total_hours, driving_hours')
      ...
      const driverIds = [...new Set(timeEntries?.map(entry => entry.driver_id))];
      ...
      timeEntries?.forEach(entry => {
        ...
        const employeeId = driver?.employee_id || 'N/A';
        ...
        const overtimeHours = Math.max(0, totalHours - 8); // Calculate overtime as hours over 8 per day
