...
const UserProfileDropdown = () => {
  ...
  // Select a random male avatar as placeholder
  const placeholderAvatar = useMemo(() => {
    const avatars = [avatarMale1, avatarMale2];
    const index = user?.id ? 
      user.id.charCodeAt(0) % avatars.length : 
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="relative h-10 w-10 rounded-full">
          <Avatar className="h-10 w-10" key={(profile?.avatar_url || placeholderAvatar) as string}>
            <AvatarImage
