...
const TransportSidebar = () => {
  ...
  // Organized navigation items with collapsible groups
  const navigationItems = [
    ...
    // Fleet Management Group
    {
      ...
      items: [
        ...
        { title: 'Tire Management', url: '/tire-management', icon: Settings },
    ...
    // Compliance Group
    {
      ...
      items: [
        ...
        { title: 'Infringement Management', url: '/infringement-management', icon: AlertTriangle },
  ...
  return (
    <Sidebar 
      className="bg-sidebar text-sidebar-text border-0"
    >
      <SidebarHeader className="px-4 py-3 border-b border-sidebar-border">
        <div className="space-y-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-lg font-semibold">transportAI</span>
              <ChevronRight className="h-4 w-4 text-sidebar-text-muted" />
            </div>
            <div className="text-sm text-sidebar-text-muted">{getUserName()}</div>
          </div>
        </div>
      </SidebarHeader>
                    ...
                    <SidebarMenuItem key={item.title}>
                      <div>
                        <button
                          ...
                          className={cn(
                            "w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-sidebar-text hover:bg-sidebar-bg-hover",
                            isExpanded && "bg-sidebar-bg-hover/50"
                          )}
                        >
                          <item.icon className="h-4 w-4 shrink-0 text-sidebar-text-muted" />
                          <span className="flex-1 text-sm text-left">{item.title}</span>
                          <ChevronRight className={cn(
                            "h-3 w-3 text-sidebar-text-muted transition-transform",
                            ...
                            {item.items.map((subItem) => {
                              ...
                              return (
                                <Link
                                  ...
                                  to={subItem.url}
                                  className={cn(
                                    "flex items-center gap-3 px-3 py-1.5 rounded-lg transition-colors text-sidebar-text hover:bg-sidebar-bg-hover",
                                    isSubActive && "bg-sidebar-bg-hover"
                                  )}
                                >
                                  <subItem.icon className={cn(
                                    "h-3.5 w-3.5 shrink-0",
                                    isSubActive ? "text-sidebar-text" : "text-sidebar-text-muted"
                                  )} />
                  ...
                  return (
                    <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton
                        asChild
                        className={cn(
                          "flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-sidebar-text hover:bg-sidebar-bg-hover",
                          isActive && "bg-sidebar-bg-hover"
                        )}
                      >
      ...
      <SidebarFooter className="px-3 py-3 mt-auto">
        <div className="space-y-1">
          <SidebarMenuButton
            asChild
            className="text-sidebar-text hover:bg-sidebar-bg-hover rounded-lg px-3 py-2"
          >
            <Link to="/settings" className="flex items-center gap-3">
              <Settings className="h-4 w-4 text-sidebar-text-muted" />
