...
serve(async (req) => {
  ...
  try {
    ...
    const { 
      ...
      monthlyPrice, 
      isAnnual = false,
      // Legacy support for planId
      planId 
    } = await req.json();
    
    // Support both new and old API
    if (!planName && !planId) throw new Error("Plan name or plan ID is required");
    ...
    // Handle legacy planId support
    let name = planName;
    let description = planDescription;
    let price = monthlyPrice;
    
    if (planId) {
      // Get plan details from subscription_plans table for legacy support
      ...
      // Use provided values
      if (isAnnual && monthlyPrice) {
        price = monthlyPrice * 10; // Annual discount (10 months for the price of 12)
      }
    }
      ...
      line_items: [
        {
          price_data: {
            ...
            unit_amount: Math.round((price || 9.99) * 100),
