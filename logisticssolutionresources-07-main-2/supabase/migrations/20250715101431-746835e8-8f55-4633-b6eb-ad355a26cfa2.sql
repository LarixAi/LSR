-- Fix the missing user_role enum type that's causing signup failures

-- First, check if the enum exists and create it if it doesn't
DO $$ 
BEGIN
...
-- Also ensure we have the organizations table structure needed
CREATE TABLE IF NOT EXISTS public.organizations (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  name TEXT NOT NULL,
  slug TEXT UNIQUE,
  contact_email TEXT,
  type TEXT CHECK (type IN ('transport_company', 'council', 'school')) DEFAULT 'transport_company',
  address TEXT,
  phone TEXT,
  email TEXT,
...
-- Now fix the handle_new_user function to properly handle the enum
