-- Comprehensive fix for mechanic signup by removing all policies that depend on role column

-- Drop ALL policies that might use the role column to avoid blocking the type change
...
DROP POLICY IF EXISTS "Admins and council can view onboarding tasks" ON public.onboarding_tasks;
DROP POLICY IF EXISTS "Admins can manage all profiles" ON public.profiles;
DROP POLICY IF EXISTS "Council can view profiles" ON public.profiles;
DROP POLICY IF EXISTS "Users can update their own profile" ON public.profiles;

-- Drop any additional policies that might reference role
DO $$ 
DECLARE
    pol RECORD;
BEGIN
    -- Find and drop policies on profiles table that might use role column
    FOR pol IN 
...
-- Update role column type to user_role enum if it's still text
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- Create essential RLS policies using security definer functions
...
USING (
  auth.uid() = id 
  OR get_current_user_role() IN ('admin', 'council')
);
...
-- Test that mechanic role exists in enum
DO $$ 
DECLARE
    role_exists BOOLEAN;
BEGIN
    ...
    RAISE NOTICE 'SUCCESS: mechanic role exists and signup should work. Try creating a mechanic account now!';
END $$;