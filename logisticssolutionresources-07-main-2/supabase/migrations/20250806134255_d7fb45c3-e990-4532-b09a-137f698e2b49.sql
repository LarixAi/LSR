-- Create tire management system tables (Fixed)
...
CREATE TABLE public.vehicle_tires (
  ...
  organization_id UUID NOT NULL,
  tire_position TEXT NOT NULL CHECK (tire_position IN ('front_left', 'front_right', 'rear_left_outer', 'rear_left_inner', 'rear_right_outer', 'rear_right_inner', 'spare')),
  ...
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  UNIQUE(vehicle_id, tire_position, status) DEFERRABLE INITIALLY DEFERRED
);
...
BEGIN
  ...
  LOOP
    ...
    SELECT 
      ...
      'Tire at position ' || tire_record.tire_position || ' has tread depth of ' || tire_record.current_tread_depth || 'mm. Legal minimum is 1.6mm.',
  ...
  LOOP
    ...
    SELECT 
      ...
      'Tire at position ' || tire_record.tire_position || ' inspection was due on ' || tire_record.next_inspection_due,
