...
SELECT 
    ...
    -- Count routes in user's organization (using status instead of is_active)
    COALESCE((
        SELECT COUNT(*)::bigint 
    ...
    -- Count active/pending jobs in user's organization
    COALESCE((
        SELECT COUNT(*)::integer 
        FROM public.jobs j
        JOIN public.profiles p ON p.organization_id = j.organization_id
        WHERE p.id = auth.uid() 
        AND j.status IN ('active', 'pending', 'in_progress')
    ), 0) AS active_jobs,
