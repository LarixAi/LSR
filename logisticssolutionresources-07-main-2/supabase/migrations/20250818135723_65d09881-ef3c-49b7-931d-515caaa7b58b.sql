-- Create a secure password reset function for affected users
...
BEGIN
    ...
    UPDATE auth.users 
    SET 
        encrypted_password = crypt(generated_password, gen_salt('bf')),
        raw_user_meta_data = COALESCE(raw_user_meta_data, '{}'::jsonb) || '{"must_change_password": true}'::jsonb,
        updated_at = now()
    WHERE id = user_id;
    
    -- Also update the profile to require password change
    UPDATE profiles 
    SET must_change_password = true
    WHERE id = user_id;
    
    RETURN QUERY SELECT true, 'Password reset successfully', generated_password;
