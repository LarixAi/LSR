...
const MobileNavigation: React.FC = () => {
  ...
  const getNavigationItems = (): NavigationItem[] => {
    ...
    if (userRole === 'driver') {
      return [
        ...
        { id: 'schedule', label: 'Schedule', icon: Calendar, path: '/driver-schedule' },
    ...
    if (userRole === 'parent') {
      return [
        ...
        { id: 'children', label: 'Children', icon: Users, path: '/child-management' },
        { id: 'tracking', label: 'Tracking', icon: MapPin, path: '/live-tracking' },
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/98 backdrop-blur-sm border-t border-gray-200 safe-area-bottom">
        ...
        {navigationItems.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex flex-col items-center justify-center px-3 py-2 rounded-lg transition-all duration-200 min-w-[68px] relative",
