-- Phase 1: Critical Foundation - Corrected RLS Policies
...
-- Jobs table policies (using assigned_to column)
CREATE POLICY "Users can view org jobs" ON public.jobs
FOR SELECT USING (organization_id IN (
  SELECT organization_id FROM public.profiles WHERE id = auth.uid()
));

CREATE POLICY "Assigned users can view their jobs" ON public.jobs
...
-- Job assignments table (linking jobs to drivers)
