...
-- 1. Fix Support Tickets Table - Remove dangerous public INSERT policy
...
USING (
  auth.uid() IS NOT NULL 
  AND (driver_id = auth.uid() OR assigned_to = auth.uid())
);
...
-- 4. Fix Orders Table - Remove dangerous INSERT with true
...
-- 6. Fix Mobile Auth Logs - Should be service role only
...
-- Create secure policies for mobile auth logs (service role access)
...
-- INSERT should be done via functions with SECURITY DEFINER

-- 7. Fix Notification Delivery Logs
...
-- 8. Fix Subscription Plans - Remove public access
...
-- 10. Fix User Agreements - Restrict public access
...
-- 12. Fix Inspection Responses - Add authentication
...
CREATE POLICY "Organization members can view responses" 
...
CREATE POLICY "Drivers can create responses" 
