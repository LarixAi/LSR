-- =====================================================
-- ADD SAMPLE DATA TO NATIONAL BUS GROUP (FINAL VERSION)
-- =====================================================
-- This script will add sample vehicles and work orders to the National Bus Group organization
-- Fixed to use correct status values and defect_number format
...
BEGIN
    ...
    -- Using status values that should be valid: 'open', 'in_progress', 'completed', 'cancelled'
    ...
    (gen_random_uuid(), 'DEF-' || LPAD(defect_counter::text, 6, '0'), national_bus_group_id, 'Engine Warning Light', 'Check engine light is on, possible sensor issue', sample_vehicle_id_1, 'electrical', 'medium', 'medium', 'open', 'Engine Bay', 150.00, 2.0, 'Driver reported warning light during morning route', existing_driver_id),
    (gen_random_uuid(), 'DEF-' || LPAD((defect_counter + 1)::text, 6, '0'), national_bus_group_id, 'Brake Pad Replacement', 'Front brake pads need replacement, squeaking noise', sample_vehicle_id_2, 'brakes', 'high', 'high', 'in_progress', 'Front Wheels', 200.00, 3.0, 'Safety critical - needs immediate attention', existing_driver_id),
    (gen_random_uuid(), 'DEF-' || LPAD((defect_counter + 2)::text, 6, '0'), national_bus_group_id, 'Air Conditioning Fault', 'AC not cooling properly, possible refrigerant leak', sample_vehicle_id_3, 'hvac', 'medium', 'medium', 'open', 'HVAC System', 300.00, 4.0, 'Passenger comfort issue', existing_driver_id),
    (gen_random_uuid(), 'DEF-' || LPAD((defect_counter + 3)::text, 6, '0'), national_bus_group_id, 'Tire Pressure Sensor', 'Tire pressure warning light on dashboard', sample_vehicle_id_1, 'electrical', 'low', 'low', 'completed', 'Wheels', 50.00, 1.0, 'Sensor replaced, all tires checked', existing_driver_id),
    (gen_random_uuid(), 'DEF-' || LPAD((defect_counter + 4)::text, 6, '0'), national_bus_group_id, 'Fuel Gauge Inaccurate', 'Fuel gauge showing incorrect readings', sample_vehicle_id_2, 'electrical', 'medium', 'medium', 'open', 'Fuel Tank', 100.00, 2.0, 'Driver cannot rely on fuel gauge', existing_driver_id);
