-- =====================================================
-- ADD SAMPLE DATA TO NATIONAL BUS GROUP (FIXED VERSION)
-- =====================================================
-- This script will add sample vehicles and work orders to the National Bus Group organization
-- Fixed to include defect_number for defect_reports
...
BEGIN
    ...
    -- 2. ADD SAMPLE WORK ORDERS (DEFECT REPORTS) WITH DEFECT_NUMBER
    ...
    (gen_random_uuid(), 'DEF-' || LPAD(defect_counter::text, 6, '0'), national_bus_group_id, 'Engine Warning Light', 'Check engine light is on, possible sensor issue', sample_vehicle_id_1, 'electrical', 'medium', 'medium', 'pending', 'Engine Bay', 150.00, 2.0, 'Driver reported warning light during morning route', existing_driver_id),
    ...
    (gen_random_uuid(), 'DEF-' || LPAD((defect_counter + 2)::text, 6, '0'), national_bus_group_id, 'Air Conditioning Fault', 'AC not cooling properly, possible refrigerant leak', sample_vehicle_id_3, 'hvac', 'medium', 'medium', 'pending', 'HVAC System', 300.00, 4.0, 'Passenger comfort issue', existing_driver_id),
    ...
    (gen_random_uuid(), 'DEF-' || LPAD((defect_counter + 4)::text, 6, '0'), national_bus_group_id, 'Fuel Gauge Inaccurate', 'Fuel gauge showing incorrect readings', sample_vehicle_id_2, 'electrical', 'medium', 'medium', 'pending', 'Fuel Tank', 100.00, 2.0, 'Driver cannot rely on fuel gauge', existing_driver_id);
