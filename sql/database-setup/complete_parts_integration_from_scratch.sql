-- =====================================================
-- COMPLETE PARTS & SUPPLIES INTEGRATION (FROM SCRATCH)
-- =====================================================
-- This script creates the complete parts and supplies integration
-- Creates ALL tables from scratch to avoid missing table/column errors

-- 1. CREATE PARTS INVENTORY TABLE (FROM SCRATCH)
...
DROP TABLE IF EXISTS public.parts_requests CASCADE;
...
-- 5. CREATE JOB PARTS TABLE (for tracking parts used in work orders)
...
-- 8. CREATE SEQUENCES
DROP SEQUENCE IF EXISTS movement_sequence;
CREATE SEQUENCE movement_sequence START 1;
...
-- 9.1 Function to generate movement numbers
...
-- 9.2 Function to generate order numbers
...
-- 9.3 Function to update part quantities automatically
...
-- 9.4 Function to create inventory alerts
...
ON CONFLICT (organization_id, part_number) DO UPDATE SET
    quantity = EXCLUDED.quantity,
    status = EXCLUDED.status,
    updated_at = NOW();

-- 12.2 Sample stock movements
...
-- 12.3 Sample parts requests (only if defect_reports table has data)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM public.defect_reports LIMIT 1) THEN
...
-- 13.1 View for parts usage in work orders
...
-- 13.2 View for parts requests summary
...
-- 13.3 View for inventory alerts
...
-- 15. CREATE SIMPLIFIED RLS POLICIES (NO profiles.organization_id references)

-- 15.1 Parts Inventory Policies
...
CREATE POLICY "Mechanics can create stock movements" ON public.stock_movements
    FOR INSERT WITH CHECK (auth.uid() IS NOT NULL AND moved_by = auth.uid());

-- 15.4 Stock Orders Policies
...
CREATE POLICY "Mechanics can create job parts" ON public.job_parts
    FOR INSERT WITH CHECK (auth.uid() IS NOT NULL AND used_by = auth.uid());

-- 15.6 Inventory Alerts Policies
...
-- 16. CREATE RLS POLICIES FOR VIEWS
...
-- NOTE: This version creates ALL tables from scratch to avoid any missing table/column errors
-- Uses simplified RLS policies that don't reference profiles.organization_id
