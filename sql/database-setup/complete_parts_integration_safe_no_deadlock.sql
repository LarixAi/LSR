-- =====================================================
-- COMPLETE PARTS & SUPPLIES INTEGRATION (DEADLOCK-SAFE)
-- =====================================================
-- This script creates the complete parts and supplies integration
-- Uses IF NOT EXISTS instead of DROP TABLE to avoid deadlocks

-- 1. CREATE PARTS INVENTORY TABLE (SAFE)
...
-- 2. CREATE PARTS REQUESTS TABLE (SAFE)
...
-- 3. CREATE STOCK MOVEMENTS TABLE (SAFE)
...
-- 4. CREATE STOCK ORDERS TABLE (SAFE)
...
-- 6. CREATE INVENTORY ALERTS TABLE (SAFE)
...
-- 12. INSERT SAMPLE DATA (SAFE - Only if not exists)
DO $$
BEGIN
    -- Only insert sample parts if the table is empty
    IF NOT EXISTS (SELECT 1 FROM public.parts_inventory LIMIT 1) THEN
...
-- 13. CREATE VIEWS FOR REPORTING (SAFE)
...
-- 15. CREATE SIMPLIFIED RLS POLICIES (SAFE - NO profiles.organization_id references)
...
-- 16. CREATE RLS POLICIES FOR VIEWS (SAFE)
DO $$
BEGIN
...
-- =====================================================
-- INTEGRATION COMPLETE (DEADLOCK-SAFE)
-- =====================================================
...
-- NOTE: This version uses IF NOT EXISTS instead of DROP TABLE to avoid deadlocks
