-- =====================================================
-- MINIMAL PARTS & SUPPLIES INTEGRATION
-- =====================================================
-- This script creates only the essential parts and supplies tables
-- No external dependencies - completely self-contained and minimal

-- 1. CREATE ORGANIZATIONS TABLE (MINIMAL)
...
-- Insert default organization if it doesn't exist (MINIMAL)
INSERT INTO public.organizations (name, slug)
VALUES ('ABC Transport Ltd', 'abc-transport')
ON CONFLICT (slug) DO NOTHING;

-- 2. CREATE PARTS INVENTORY TABLE (MINIMAL)
...
-- 3. CREATE PARTS REQUESTS TABLE (MINIMAL)
...
-- 4. CREATE STOCK MOVEMENTS TABLE (MINIMAL)
...
-- 5. CREATE STOCK ORDERS TABLE (MINIMAL)
...
-- 6. CREATE JOB PARTS TABLE (MINIMAL)
...
-- 7. CREATE INVENTORY ALERTS TABLE (MINIMAL)
...
-- 8. ENABLE ROW LEVEL SECURITY (MINIMAL)
...
-- 9. CREATE BASIC INDEXES (MINIMAL)
...
-- 13. CREATE BASIC RLS POLICIES (MINIMAL)
DO $$
BEGIN
    -- Organizations
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'parts_inventory' AND policyname = 'Users can manage parts inventory') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'parts_requests' AND policyname = 'Users can manage parts requests') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'stock_movements' AND policyname = 'Users can manage stock movements') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'stock_orders' AND policyname = 'Users can manage stock orders') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'job_parts' AND policyname = 'Users can manage job parts') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'inventory_alerts' AND policyname = 'Users can manage inventory alerts') THEN
        CREATE POLICY "Users can manage inventory alerts" ON public.inventory_alerts
            FOR ALL USING (auth.uid() IS NOT NULL);
    END IF;
END $$;

-- 14. CREATE RLS POLICIES FOR VIEWS (MINIMAL)
DO $$
BEGIN
...
-- =====================================================
-- The Parts & Supplies system is now minimally integrated with:
-- 1. Organizations table (minimal structure)
...
-- NOTE: This version is minimal and focused
-- No complex functions, triggers, or external dependencies
-- No references to schools table or other external tables
-- Simple RLS policies that only check authentication
