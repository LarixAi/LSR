-- =====================================================
-- PARTS & SUPPLIES INTEGRATION SYSTEM (FIXED VERSION)
-- =====================================================
-- This script creates the complete parts and supplies integration
-- with work orders, vehicle inspections, and maintenance systems

-- 1. ENHANCE PARTS INVENTORY TABLE
...
-- Make organization_id NOT NULL after populating (only if it exists)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
...
BEGIN
    ...
    IF NEW.quantity <= NEW.min_quantity AND NEW.quantity > 0 THEN
        INSERT INTO public.inventory_alerts (
            ...
            COALESCE(NEW.organization_id, (SELECT id FROM public.organizations WHERE name = 'ABC Transport Ltd' LIMIT 1)),
            NEW.id,
            'low_stock',
