-- =====================================================
-- PARTS & SUPPLIES INTEGRATION SYSTEM (SAFE VERSION)
-- =====================================================
-- This script safely creates the complete parts and supplies integration
-- Step 1: Add organization_id column first
-- Step 2: Create all tables
-- Step 3: Add RLS policies after columns exist

-- STEP 1: ENHANCE PARTS INVENTORY TABLE FIRST
-- Add missing columns and organization support
DO $$
BEGIN
...
-- STEP 2: CREATE ALL TABLES (without RLS policies yet)
...
-- STEP 4: CREATE RLS POLICIES (now that all columns exist)
...
-- Function to update part quantities automatically
...
BEGIN
    ...
    ELSE
        SELECT id INTO org_id FROM public.organizations WHERE name = 'ABC Transport Ltd' LIMIT 1;
    END IF;
...
-- Sample parts inventory (only if organization_id column exists)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- Sample parts requests (only if defect_reports table has data)
DO $$
BEGIN
...
-- View for parts usage in work orders
...
-- View for parts requests summary
...
-- STEP 12: CREATE RLS POLICIES FOR VIEWS
