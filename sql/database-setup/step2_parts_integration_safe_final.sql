-- =====================================================
-- STEP 2: PARTS & SUPPLIES INTEGRATION (SAFE FINAL VERSION)
-- =====================================================
...
-- 10. CREATE RLS POLICIES (SAFE VERSION - Check if profiles has organization_id first)

-- Check if profiles table has organization_id column before creating policies
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'RLS policies created successfully using profiles.organization_id';
    ELSE
        -- Fallback: Create simple policies without organization_id reference
        RAISE NOTICE 'profiles table does not have organization_id column. Creating basic RLS policies.';
        
        -- Basic policies that allow authenticated users to access their own data
        DROP POLICY IF EXISTS "Basic access policy" ON public.parts_requests;
        CREATE POLICY "Basic access policy" ON public.parts_requests FOR ALL USING (auth.uid() IS NOT NULL);
        
        DROP POLICY IF EXISTS "Basic access policy" ON public.stock_movements;
        CREATE POLICY "Basic access policy" ON public.stock_movements FOR ALL USING (auth.uid() IS NOT NULL);
        
        DROP POLICY IF EXISTS "Basic access policy" ON public.stock_orders;
        CREATE POLICY "Basic access policy" ON public.stock_orders FOR ALL USING (auth.uid() IS NOT NULL);
        
        DROP POLICY IF EXISTS "Basic access policy" ON public.job_parts;
        CREATE POLICY "Basic access policy" ON public.job_parts FOR ALL USING (auth.uid() IS NOT NULL);
        
        DROP POLICY IF EXISTS "Basic access policy" ON public.inventory_alerts;
        CREATE POLICY "Basic access policy" ON public.inventory_alerts FOR ALL USING (auth.uid() IS NOT NULL);
    END IF;
END $$;
