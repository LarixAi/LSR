-- =====================================================
-- ENHANCE DEFECT REPORTS SYSTEM (FIXED)
-- =====================================================
...
-- 7. Create a simplified view to combine defect reports and vehicle check defects
...
SELECT 
    ...
    'Vehicle Check Defect - ' || COALESCE(vc.status, 'General') as title,
    'Defect found during vehicle check on ' || vc.check_date::text as description,
    'safety' as defect_type,
    CASE 
        WHEN vc.status = 'failed' THEN 'critical'
        WHEN vc.status = 'flagged' THEN 'high'
        ELSE 'medium'
    END as severity,
    CASE 
        WHEN vc.status = 'passed' THEN 'resolved'
        WHEN vc.status = 'failed' THEN 'investigating'
        ELSE 'reported'
    END as status,
    'Vehicle' as location,
