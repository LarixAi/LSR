-- Fix profiles table RLS policies and structure to prevent 400 errors for parent users
-- Run this in the Supabase SQL editor

-- 1. Drop the overly permissive policies that were created in the recent migration
DROP POLICY IF EXISTS "Enable read access for authenticated users" ON public.profiles;
DROP POLICY IF EXISTS "Enable insert for authenticated users" ON public.profiles;
DROP POLICY IF EXISTS "Enable update for authenticated users" ON public.profiles;

-- 2. Check if the profiles table has the wrong primary key structure
DO $$
BEGIN
...
-- 6. Log the fix (if audit_logs table exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_logs') THEN
        INSERT INTO public.audit_logs (action, table_name, user_id, organization_id, new_values)
        VALUES (
          'FIX_PROFILES_400_ERROR', 
          'profiles', 
          auth.uid(),
          NULL,
          '{"description": "Fixed profiles table structure and RLS policies to prevent 400 errors for parent users", "issue": "profiles_400_error"}'::jsonb
        );
    END IF;
END $$;
