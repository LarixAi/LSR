...
SELECT
  ...
  '{
    ...
    "items": ["Soft brake pedal", "Reduced braking power", "Safety concern"]
  }'::jsonb as defects_details,
...
-- 14. Show the created data
SELECT 'Inspection Templates Created:' as info;
SELECT name, inspection_type, frequency_days FROM inspection_templates;

SELECT 'Inspection Schedules Created:' as info;
SELECT 
  s.id,
  s.scheduled_date,
  s.status,
...
SELECT 'Vehicle Inspections Created:' as info;
SELECT 
  vi.id,
  vi.inspection_type,
  vi.inspection_category,
  vi.overall_status,
  vi.defects_found,
  vi.inspection_date,
