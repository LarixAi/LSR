-- Comprehensive Tachograph Database Setup
-- This script creates all necessary tables and adds sample data

-- 1. Create tachograph_records table
...
-- 2. Create tachograph_folders table
...
-- 3. Create tachograph_card_readers table
...
-- 4. Create tachograph_download_sessions table
...
CREATE POLICY "Users can view tachograph folders from their organization" ON public.tachograph_folders
  FOR SELECT USING (organization_id IN (
    SELECT organization_id FROM public.profiles WHERE id = auth.uid()
  ));
...
CREATE POLICY "Users can view tachograph card readers from their organization" ON public.tachograph_card_readers
  FOR SELECT USING (organization_id IN (
    SELECT organization_id FROM public.profiles WHERE id = auth.uid()
  ));
...
CREATE POLICY "Users can view tachograph download sessions from their organization" ON public.tachograph_download_sessions
  FOR SELECT USING (organization_id IN (
    SELECT organization_id FROM public.profiles WHERE id = auth.uid()
  ));
...
-- 12. Show sample tachograph records
SELECT 
    tr.id,
    tr.record_date,
    tr.activity_type,
