-- =====================================================
-- VEHICLE CHECK QUESTIONS INTEGRATION SYSTEM
-- =====================================================
-- This script creates a comprehensive integration between 
-- the mobile Enhanced Vehicle Check questions and the 
-- admin Smart Inspections system
-- =====================================================

-- Drop existing tables if they exist (for clean setup)
DROP TABLE IF EXISTS "public"."inspection_questions" CASCADE;
DROP TABLE IF EXISTS "public"."inspection_question_sets" CASCADE;

-- Create inspection question sets table
CREATE TABLE "public"."inspection_question_sets" (
...
-- Create inspection questions table
...
CREATE POLICY "Admins can manage question sets" ON "public"."inspection_question_sets"
    FOR ALL USING (
        organization_id IN (
...
CREATE POLICY "Admins can manage questions" ON "public"."inspection_questions"
    FOR ALL USING (
        question_set_id IN (
            SELECT id FROM inspection_question_sets 
            WHERE organization_id IN (
...
CREATE INDEX "idx_inspection_question_sets_org_id" ON "public"."inspection_question_sets"("organization_id");
CREATE INDEX "idx_inspection_question_sets_created_by" ON "public"."inspection_question_sets"("created_by");
CREATE INDEX "idx_inspection_questions_set_id" ON "public"."inspection_questions"("question_set_id");
CREATE INDEX "idx_inspection_questions_order" ON "public"."inspection_questions"("order_index");
...
-- =====================================================
-- COMPREHENSIVE VEHICLE CHECK QUESTIONS
-- =====================================================
-- These are the complete 56 questions from the mobile 
-- Enhanced Vehicle Check system
-- =====================================================

-- Function to create default question set for an organization
CREATE OR REPLACE FUNCTION create_default_daily_pretrip_questions(
    org_id UUID,
    creator_id UUID
)
RETURNS UUID AS $$
...
BEGIN
    ...
    -- Insert all 56 comprehensive questions
...
-- =====================================================
-- This function automatically creates default questions 
-- for new organizations
-- =====================================================

CREATE OR REPLACE FUNCTION create_default_questions_for_new_org()
RETURNS TRIGGER AS $$
DECLARE
    admin_user_id UUID;
BEGIN
    -- Find an admin user for this organization (the creator)
    ...
    -- If we found an admin user, create default questions
    IF admin_user_id IS NOT NULL THEN
        PERFORM create_default_daily_pretrip_questions(NEW.id, admin_user_id);
    END IF;
...
-- Create trigger to auto-create questions for new organizations
DROP TRIGGER IF EXISTS auto_create_default_questions ON organizations;
CREATE TRIGGER auto_create_default_questions
    AFTER INSERT ON organizations
    FOR EACH ROW
    EXECUTE FUNCTION create_default_questions_for_new_org();

-- =====================================================
-- MANUAL SETUP FOR EXISTING ORGANIZATIONS
-- =====================================================
-- Run this for existing organizations that need the 
-- default question set
-- =====================================================

-- Example: Create default questions for a specific organization
-- Replace 'your-org-id' and 'admin-user-id' with actual values
-- SELECT create_default_daily_pretrip_questions('your-org-id', 'admin-user-id');
