...
const BudgetOverview = () => {
  ...
  const { data: budgetData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Compute a conservative weekly budget without DB dependencies
      const driverWages = 0;
      const fuel = 0;
      const maintenance = 0;
      const other = 0;
      ...
      return {
        weekly: {
          total: driverWages + fuel + maintenance + other,
        ...
        incoming: [
          { source: 'School Contract', amount: 0, status: 'confirmed' },
          { source: 'Private Events', amount: 0, status: 'confirmed' },
          { source: 'Airport Transfers', amount: 0, status: 'confirmed' }
        ],
        outgoing: [
          { category: 'Driver Wages', amount: Math.round(driverWages), status: 'scheduled' },
          { category: 'Fuel Costs', amount: fuel, status: 'recurring' },
          { category: 'Vehicle Maintenance', amount: maintenance, status: 'pending' },
          { category: 'Insurance', amount: Math.round(driverWages * 0.08), status: 'monthly' }
        ]
      };
    },
