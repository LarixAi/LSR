...
const TransportRegistrationForm = () => {
  ...
  const { data: transportOptions } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Mock transport routes data since the routes table doesn't exist
      const mockRoutes: MockRoute[] = [
        {
          id: 'route-1',
          name: 'City Center - Lincoln Elementary',
          transport_company: 'Logistics Solution Resources',
          route_number: 'R001',
          is_active: true
        },
        {
          id: 'route-2',
          name: 'Suburban Express - Lincoln Elementary',
          transport_company: 'Logistics Solution Resources',
          route_number: 'R002',
          is_active: true
        },
        {
          id: 'route-3',
          name: 'School Run Special - Lincoln Elementary',
          transport_company: 'Logistics Solution Resources',
          route_number: 'R003',
          is_active: true
        },
        {
          id: 'route-4',
          name: 'North Route - Washington High',
          transport_company: 'Logistics Solution Resources',
          route_number: 'R004',
          is_active: true
        }
      ...
      const companies = [...new Set(mockRoutes.map(r => r.transport_company))];
      return { companies, routes: mockRoutes };
    }
  });
  ...
  const registerChildMutation = useMutation({
    mutationFn: async () => {
      ...
      const selectedRoute = transportOptions?.routes.find(
        r => r.transport_company === formData.transportCompany && 
            r.route_number === formData.routeNumber
      );
                ...
                >
                  ...
                  <SelectContent>
                    {availableRoutes.map((route) => (
                      <SelectItem key={route.id} value={route.route_number}>
