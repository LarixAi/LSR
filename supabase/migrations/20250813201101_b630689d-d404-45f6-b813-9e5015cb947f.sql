-- Temporarily disable the profile change prevention trigger
DROP TRIGGER IF EXISTS prevent_privilege_change ON public.profiles;

-- Fix the handle_new_user function to create proper user profiles instead of test-like defaults
...
BEGIN
    ...
    INSERT INTO public.profiles (
        ...
        CASE 
            WHEN is_admin_user THEN 'active'
            WHEN user_role_value = 'admin' THEN 'active'
            ELSE 'active'  -- Changed from 'applicant' to 'active'
        END,
        CASE 
            WHEN is_admin_user THEN 'completed'
            WHEN user_role_value = 'admin' THEN 'completed'
            ELSE 'in_progress'  -- Changed from 'pending' to 'in_progress'
        END,
        true,
        target_org_id,
        now(),
...
-- Update existing profiles that have 'applicant' status to 'active'
UPDATE public.profiles 
SET employment_status = 'active',
    onboarding_status = 'in_progress',
    updated_at = now()
WHERE employment_status = 'applicant' 
   OR onboarding_status = 'pending';

-- Re-enable the profile change prevention trigger with improved logic
