...
BEGIN
  ...
  SELECT 
    ...
    'Document expires on ' || dd.expiry_date || ': ' || dd.name
  FROM public.driver_documents dd
  WHERE dd.expiry_date BETWEEN CURRENT_DATE AND (CURRENT_DATE + INTERVAL '30 days')
    AND dd.status = 'approved'
    AND NOT EXISTS (
      SELECT 1 FROM public.document_notifications dn
      WHERE dn.document_id = dd.id
        AND dn.type = 'document_due_soon'
        AND dn.created_at > (CURRENT_DATE - INTERVAL '7 days')
    );
END;
$$ LANGUAGE plpgsql;
